...
class FaceDetect(object):
    ...
    def load_images(self):
        ...
        train_images = tf.data.Dataset.list_files(os.path.join(self.input_db_path, 'aug_data', 'train', 'images', '*.jpg'), shuffle=False)
        train_images = train_images.map(self.load_image)
        train_images = train_images.map(lambda x: tf.image.resize(x, (500, 500)))
        train_images = train_images.map(lambda x: x / 255)

        test_images = tf.data.Dataset.list_files(os.path.join(self.input_db_path, 'aug_data', 'test', 'images', '*.jpg'), shuffle=False)
        test_images = test_images.map(self.load_image)
        test_images = test_images.map(lambda x: tf.image.resize(x, (500, 500)))
        test_images = test_images.map(lambda x: x / 255)

        val_images = tf.data.Dataset.list_files(os.path.join(self.input_db_path, 'aug_data', 'val', 'images', '*.jpg'), shuffle=False)
        val_images = val_images.map(self.load_image)
        val_images = val_images.map(lambda x: tf.image.resize(x, (500, 500)))
        val_images = val_images.map(lambda x: x / 255)

        train_labels = tf.data.Dataset.list_files(os.path.join(self.input_db_path, 'aug_data', 'train', 'labels', '*.json'), shuffle=False)
        train_labels = train_labels.map(lambda x: tf.py_function(self.load_labels, [x], [tf.uint8, tf.float16]))

        test_labels = tf.data.Dataset.list_files(os.path.join(self.input_db_path, 'aug_data', 'test', 'labels', '*.json'), shuffle=False)
        test_labels = test_labels.map(lambda x: tf.py_function(self.load_labels, [x], [tf.uint8, tf.float16]))

        val_labels = tf.data.Dataset.list_files(os.path.join(self.input_db_path, 'aug_data', 'val', 'labels', '*.json'), shuffle=False)
        val_labels = val_labels.map(lambda x: tf.py_function(self.load_labels, [x], [tf.uint8, tf.float16]))

        logger.info("len(train_images): {}, len(train_labels): {}, len(test_images): {}, len(test_labels): {}, len(val_images): {}, len(val_labels): {}".format(
        ...
        self.test1 = tf.data.Dataset.zip((test_images, test_labels))
        self.test1 = self.test1.shuffle(1300)
        self.test1 = self.test1.batch(8)
        self.test1 = self.test1.prefetch(4)
            ...
            for image in os.listdir(os.path.join(self.input_db_path, partition, 'data')):
                ...
                try:
                    for x in range(60):
                        ...
                        cv2.imwrite(os.path.join(self.input_db_path,'aug_data', partition, 'images', f'{image.split(".")[0]}.{x}.jpg'),
                                    augmented['image'])
                        ...
                        with open(os.path.join(self.input_db_path, 'aug_data', partition, 'labels', f'{image.split(".")[0]}.{x}.json'),
                                  'w') as f:
