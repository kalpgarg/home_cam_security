...
class CaptureFace(object):
    ...
    def save_capture(self, log_folder, name, total_capture_cnt, sleep_t):
...
if __name__ == '__main__':
    ...
    capture_face_args.add_argument('-t', '--time_bw_capture', action='store', type=int, default=1,
                                 metavar='1', help='Time(in milliseconds) between two captures. Not used for pre-recorded videos')
    capture_face_args.add_argument('-p', '--person_name', action='store', type=str, required=True,
                                 metavar='person1', help='person name')
    capture_face_args.add_argument('-d', '--is_dir', action='store', type=bool, default=False, metavar='False', help='Whether source is a directory which contain multiple videos')
